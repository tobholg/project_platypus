...
fn fragment(in: FullscreenVertexOutput) -> @location(0) vec4<f32> {
    ...
    // Swirl: more toward the middle, breathing a little.
    let breathe = 1.0 + 0.12 * sin(warp.time * 3.1 + r * 9.0);
    let angle = warp.strength * 3.2 * fall * breathe;
    let c = cos(angle);
    let s = sin(angle);
    var q = vec2<f32>(d.x * c - d.y * s, d.x * s + d.y * c);
    ...
    let heart = smoothstep(0.32, 0.0, r) * 0.55 * warp.strength;
    let ring = exp(-pow((r - 0.78) * 14.0, 2.0)) * 0.22 * warp.strength;

...
Space in the air to thrust up (3 s, refilled on landing; the exhaust is fire that burns
what's under you and lights grass and wood); cloud boots (in the pack) give a triple
jump. E throws the grappling hook (worn in the Hook slot from the start while
developing) at the cursor: it grabs rock, platforms, chests, bodies and
creatures. Hanging from it, A/D swing, W climbs, S lets out rope, holding E reels
in, Space lets go (speed kept); the rope wraps round corners, and comes loose if
you dig out what it holds. Something smaller than you (a chest, a body, a bat) is
pulled to you instead. Gear (armour, trinkets) goes in the
...
carried (the chest window; R takes all); an emptied body fades. Enemies wear and wield gear
(`wears` in their creature file, their weapon as an item), rolled where they spawn: what they
wear is what they drop. Uniques (the Troll's Club, the Broodmother's Fang: venom on hit) are
legendary and rare. Mining and building work in 4 × 4-cell blocks;
...
speed. Each focus glows and gives off sparks at its tip (its `aura` in
items.ron: embers, frost mist, crackle, acid drips), more while casting.
Casting costs mana (the blue bar, refilling). A spell is a list of runes
(`assets/data/runes.ron`, hot-reloaded): a carrier (bolt, orb, arrow, flame
stream, lightning), what it carries (spark, blast, ignite, heat, frost, acid,
water) and modifiers (heavy, fire trail, quick, trigger). The gravity
...
water into ice you can stand on (the frost wand, hotbar 2) and slide on (ice is slippery for
every creature; it doesn't chill you, a frost bolt does); bolts fizzle out
...
Scenarios: `idle`, `pan`, `avalanche`, `run`, `tools`, `tree`, `blast`, `fell`, `burn`, `acid`, `rain`, `swim`, `night`, `dusk`, `cave`, `flood` (a block of water collapsing in a dug hall), `strike` (lightning onto the nearest tree), `hands` (digs a shaft, builds, chops with auto tool, plants a torch), `chest` (places, opens, fills and breaks a chest), `chestfall` (digs out a chest's floor: it falls), `drop` (holds S on a platform), `magic` (every starter wand at orcs; `PLATYPUS_WORLD=flat` for a clear view), `shock` (lightning into a pool with orcs in it, flat world), `inventory` (opens it, hovers a wand, drags it to hotbar 3; moves the real mouse pointer), `well` (the gravity wand lifts the ground and two orcs, swings, drops it; flat world), `force` (pushes a sand pile and orcs away, then pulls; flat world), `splash` (spells into a pool and an oil pit; flat world), `critters` (a rabbit, a bird and a frog walked at: they flee), `arena` (`PLATYPUS_WORLD=arena`: wands at the dummies, an orc spawned, pause and three steps, quarter speed; logs ticks and readouts), `webs` (arena world: a walk through open air, then through web; logs both), `nest` (small world: into the nearest spider nest with a torch; logs what's about), `underground` (arena world: each underground enemy in turn against a player standing still, then a spider climbing a column; logs what each cost), `life` (arena world, try `PLATYPUS_HOUR=22`: fireflies, fish, bats; logs whether each stays where it lives), `crossing` (arena world: through a falling sand stream; out of a pit of water by jumping at the surface), `held` (arena world: the pickaxe into the floor, the torch, a wand casting, a bomb thrown, the axe; logs what's wielded), `warband` (arena world: O spawns the default pack; logs what came), `archery` (arena world: the bow at a dummy, into the floor, through lava, the arrows picked up, an orc archer), `fight` (arena world: the shortsword against an orc, then a troll; logs both sides), `melee` (arena world: both swords at a dummy, a pogo, a blast mid-dodge; logs hits, stamina and hp), `wands` (arena world: the spark wand into the floor and at a sandbag, the flame wand at dummies 60 and 120 cells off), `gear` (arena world: a blast with nothing on, then in full iron; logs both and the stats; rolls a piece of each rarity; opens the equipment screen), `loot` (arena world: an orc in gear struck dead; logs its body's contents and right-clicks it open), `fang` (arena world: the Broodmother's Fang at an orc; logs its health and its venom), `chaos` (a stress test, best in the arena world; in a generated world it fights where the player starts, `PLATYPUS_CHAOS_RUN=1` moving right to stream the world in: waves of mixed enemies every 3 s, `PLATYPUS_CHAOS`=8 a wave × its number, the player casting at them; bombs, sand, water, lava and lightning; logs creatures, bodies, particles, sparks and mesh/image uploads each second; with `--features spikes` and `PLATYPUS_PROFILE=5` the heaviest systems every 5 s), `spider` (arena world: a cave spider against a player standing still; logs its spits, bites and stings and what they cost), `rocket` (arena world: rocket boots over an orc on planks; logs the climb, the orc and the fire), `hook` (arena world: the grappling hook leashes a chest and reels it in, swings from a stone beam round a hanging block, comes loose when the beam is dug out, hooks on again and lets go mid-swing; logs each), `ice` (arena world: frost bolts freeze the pool; a run and slide on it vs on stone; a frost bolt chills an orc), `foci` (arena world, `PLATYPUS_SLOT`=1–30, try `PLATYPUS_HOUR=22`: holds a focus out to see its aura; `PLATYPUS_CAST=1` casts), `editor` (arena world, `PLATYPUS_EDIT_DIR` = a folder of sprite COPIES: paints a stroke with the real pointer, undoes it, logs both). Bench scenarios: `settled`, `deep`, `avalanche`, `streaming`. Add `PLATYPUS_OFFSCREEN=1` for screenshots that work with the screen locked (the camera draws into an image; no HUD). Add `PLATYPUS_SCREENSHOT=out.png` to
...
- **A lair** (a nest, a pit): an entry in `assets/data/lairs.ron` (how deep, which underground biomes, how often, what lines it, who lives there). New worlds (and unvisited chunks) have it.
- **An underground enemy:** copy one of `spider`, `slime`, `vampire_bat`, `skeleton`, `egg_sac` in `assets/data/creatures/`: its brain (`crawler`, `hopper`, `swooper`, `melee_walker`, `hatchery`), `touch` for contact damage, `cling` in its movement to climb walls and ceilings.

...
PLATYPUS_WORLD=arena cargo run -p platypus --release # the arena: dummies, pause/step/slow motion (P . ,), boxes (Y), O spawns the kind picked in its panel
...
to the dev tools and back · swords: the shortsword and longsword start on hotbar 2 (X, then 7 or
8): hold LMB to swing at the cursor through the combo, strike down in the air to bounce off

...
type Exposed<'a> = (Entity, &'a Kinematics, &'a mut Health, Option<&'a Resist>, Option<&'a mut Burning>, Option<&'a mut Wet>);
...
pub fn expose(mut commands: Commands, mut sim: ResMut<SimWorld>, mut q: Query<Exposed>) {
    ...
    for (entity, k, mut health, resist, burning, wet) in &mut q {
...
/// Burning creatures flicker orange; wet ones look a little blue.
pub fn tint(
    time: Res<Time>,
    creatures: Query<(&Children, Has<Burning>, Has<Wet>)>,
    mut sprites: Query<&mut Sprite, With<CreatureSprite>>,
) {
    let t = time.elapsed_secs();
    for (children, burning, wet) in &creatures {
